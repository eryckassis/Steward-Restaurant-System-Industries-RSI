...
export function LowStockAlerts() {
  ...
  const getRecommendation = (item: InventoryItem) => {
    ...
    if (status.status === "critical") {
      return `Reabastecer urgente: +${formatQuantityWithUnit(
        Math.max(deficit * 2, item.min_stock),
        item.unit
      )}`;
    }
    if (status.status === "low") {
      return `Considerar compra: +${formatQuantityWithUnit(
        Math.max(deficit * 1.5, item.min_stock * 0.5),
        item.unit
      )}`;
    }
              ...
              <div className="pt-3 border-t mt-3">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-medium">
                    {formatCurrency(totalAtRisk)} em estoque
                  </span>
                </div>
              </div>
            )}
