  ...
  try {
    ...
    if (!wasteError && wasteData) {
      wasteData.forEach((w) => {
        ...
        const key = `${date.getFullYear()}-${date.getMonth()}`;
        const cost =
          typeof w.cost === "number" && !isNaN(w.cost) && w.cost >= 0
            ? w.cost
            : 0;
        if (monthlyWaste.has(key)) {
          monthlyWaste.set(key, (monthlyWaste.get(key) || 0) + cost);
        }
      });
    }
    ...
    if (!movementsError && movements) {
      movements.forEach((m) => {
        ...
        const key = `${date.getFullYear()}-${date.getMonth()}`;
        const cost =
          typeof m.cost === "number" && !isNaN(m.cost) && m.cost >= 0
            ? m.cost
            : 0;
        if (monthlyWaste.has(key)) {
          monthlyWaste.set(key, (monthlyWaste.get(key) || 0) + cost);
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const value = monthlyWaste.get(key) || 0;
    ...
    console.error("[v0] Error in waste chart API:", error);
